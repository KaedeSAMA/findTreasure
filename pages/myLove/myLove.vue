<template>
	<view class="myLoveRoot" :style="'min-height:'+(screenHeight)+'px'">
		<view class="title">
			...
			<view class="mainCards" v-show="cardListL[0] != null">
				<view class="flowL" ref="flowL">
					<card v-for="item in cardListL" :key="item.id" class="card" :imgURL="item.fullImg" :treasureId="item.id" :treasureName="item.name"
						:location="item.location" :charList="item.charList" :isLike="item.isLove"></card>
				</view>
				<view class="flowR" ref="flowR">
					<card v-for="item in cardListR" :key="item.id" class="card" :imgURL="item.fullImg" :treasureId="item.id" :treasureName="item.name"
						:location="item.location" :charList="item.charList" :isLike="item.isLove"></card>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods:{
			...
			getLoveList(){
				uni.$http.get("/personal/collection").then((res)=>{
					console.log(res.data);
					res.data.data.collectionList.reverse();
					let i = 1; 
					res.data.data.collectionList.map(val => {
